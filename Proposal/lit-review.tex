...
A similar obstacle is met when applying deep learning architecture to recover galaxy parameters. Compared to shallow neural networks which have a single hidden layer controlling relationships between variables, deep neutral networks contain several hidden layers with interconnected dependencies. As visualised in Fig. - tens or even hundreds of inputs nodes result in the modelling of only one or two free parameters, 
...
\section{\textit{The Cannon}: Machine-Learning for Stars, Applied to Galaxies}
\label{sec:tc}

\subsection{SED Possibilities}
